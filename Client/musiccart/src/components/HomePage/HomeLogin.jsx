...
import ViewListCard from '../Card/ViewListCard';
...
function HomeLogin() {
  ...
  useEffect(() => {
    fetchData();
    const user = localStorage.getItem('name'); // Retrieve user data from local storage
    if (user) {
      setUserName(user); 
    }
  ...
  function stringAvatar(name) {
    return {
      sx: {
        ...
        color:"black" // Use the name to generate background color
      },
      children: name ? `${name.split(' ')[0][0]}${name.split(' ')[1][0]}` : 'U', // Use initials if name is available
    };
  }
            ...
            <section className={styles.mainChild}>
              ...
              <div className={styles.musicartWrapper}>
               ...
               <div className={styles.topHome}>
                   ...
                   <Button className={styles.home} style={{color:"black"}}>
                    Invoice
                  </Button>
                 </div>
                ...
                <div className={styles.rightContent}>
                  ...
                  <Link to="/viewCart" className={styles.cartButton}>
                    <ShoppingCartOutlinedIcon style={{paddingLeft:"10px",width:"18px", height:"18px"}}/><span> View cart {cartCount}</span>
                  </Link>
              ...
              <div className={styles.filterContent}>
                ...
                <WindowIcon onClick={handleGridModeClick} />
                <img src={mobileView} alt='mobile' className={styles.mobileView} onClick={handleListModeClick}/>
                 </div>
              ...
              {items.map((item) => (
                viewMode === 'gridView' ? (
                  <Card
                    ...
                    price={item.price}
                    colorOrType={`${item.color} | ${item.type}`}
